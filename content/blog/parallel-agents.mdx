...
Any read one task needs from another is satisfied by a contract fixed *before* dispatch, not by a live read of the other task's in-progress file. Violate that and you're not parallelizing, you're racing.
...
Each one ran `tsc --noEmit` on its own slice before reporting back. The only real merge risk was `globals.css`, since two agents both needed to append to it. We solved that by telling both to *read fresh right before editing* instead of writing from a stale copy, so appends never clobber each other.
